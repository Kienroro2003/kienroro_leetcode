...
// 
//
// Related Topics Array Stack Simulation 👍 3357 👎 1969
...
public class BaseballGame {
    public static void main(String[] args) {
        BaseballGame outer = new BaseballGame();
    ...
    class Solution {
        public int calPoints(String[] operations) {
            ...
            for (String operation : operations) {
                switch (operation) {
                    case "+" -> {
                        int newScore = scores.get(scores.size() - 1) + scores.get(scores.size() - 2);
                        scores.add(newScore);
                        totalScore += newScore;
                    }
                    case "C" -> {
                        int removedScore = scores.remove(scores.size() - 1);
                        totalScore -= removedScore;
                    }
                    case "D" -> {
                        int newScore = scores.get(scores.size() - 1) * 2;
                        scores.add(newScore);
                        totalScore += newScore;
                    }
                    default -> {
